...
class EmailHandler:
    ...
    def customer_login_email(self):
        ...
        html_body = render_to_string("mails/customer_login_mail.html", self.mail_body)
        #send_mail(self.mail_subject, message, self.from_mail, self.to_mail)
        msg = EmailMultiAlternatives(subject=self.mail_subject, from_email=self.from_mail,
                             to=self.to_mail, body=text_body)
